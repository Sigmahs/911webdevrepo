...
from emotions.models import Document, PictureSubmission, VideoSubmission, Thread, Comment, PictureUser, VideoUser
from emotions.forms import DocumentForm, ThreadForm, CommentForm, UserVideoForm
...
def home(request):
    ...
    if user.is_active:
        return render(request, 'emotions/home.html', {"firstTime": firstTime})
    else:
...
def video(request):
    return render(request, 'emotions/video.html')
...
def submit(request):
    ...
    if subs >= 5 and not (user.is_superuser):
        return render(request,'emotions/fail2.html')
    ...
    if scores == [] :
        posted["success"] = 0
    return render(request,'emotions/results.html' ,{"posted":posted});
...
def faceTracker(request):
    ...
    return render(request, 'emotions/overlay.html',{'left':left,'up':up,'width':width,'height':height})



def overlay(request):
    return render(request, 'emotions/overlay.html');

def results_overlay(request):
    return render(request, 'emotions/results_overlay.html');
...
# Convert_Single_Img
    try:
        ...
        return render(request,'emotions/fail.html')
    ...
    try:
        ...
        return render(request,'emotions/results_overlay.html',{'posted': posted})
    except:
        return render(request,'emotions/fail2.html')
...
def forum(request):
    ...
    return render(request, 'emotions/forum.html', {'threads': threads, 'threadForm':threadForm, 'commentForm': commentForm, 'comments':comms})
...
def userVideo(request):
    ...
    form = UserVideoForm()
    data["form"] = form
    return render(request,'emotions/userVideo.html',data)
